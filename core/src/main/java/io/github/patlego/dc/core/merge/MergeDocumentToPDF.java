...
/**
 * This sample illustrates how to merge the Word based document template with
 * the input JSON data to generate the output document in the PDF format.
 * <p>
...
public class MergeDocumentToPDF implements DoCService {
    ...
    public OutputStream invoke() throws DocCloudException {
        ...
        try {
            ...
            String content = IOUtils.toString(this.getClass().getClassLoader().getResourceAsStream("salesOrder.json"),
                    StandardCharset.UTF_8);
            JSONObject jsonDataForMerge = new JSONObject(content);

            // Create an ExecutionContext using credentials.
            ExecutionContext executionContext = ExecutionContext.create(this.getCredentials());

            // Create a new DocumentMergeOptions instance
            DocumentMergeOptions documentMergeOptions = new DocumentMergeOptions(jsonDataForMerge, OutputFormat.PDF);

            // Create a new DocumentMergeOperation instance with the DocumentMergeOptions
            // instance
            ...
            FileRef documentTemplate = FileRef.createFromStream(
                    this.getClass().getClassLoader().getResourceAsStream("salesOrderTemplate.docx"),
    ...
    public Credentials getCredentials() throws Exception {
        JSONObject config = new JSONObject(IOUtils.toString(
                this.getClass().getClassLoader().getResourceAsStream("pdfservices-api-credentials.json"),
                StandardCharset.UTF_8));
        ...
        Credentials credentials = Credentials.serviceAccountCredentialsBuilder()
                ...
                .withPrivateKey(IOUtils.toString(this.getClass().getClassLoader().getResourceAsStream("private.key"),
                        StandardCharset.UTF_8))
                .build();
        return credentials;
